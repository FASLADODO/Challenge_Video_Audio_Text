...
from os import sep
from Yin import audio_processing
import time
...
def compute_yin(sig, sr, dataFileName=None, w_len=512, w_step=256, f0_min=100, f0_max=500, harmo_thresh=0.4):
    """
...
def main(audioFileName="whereIam.wav", w_len=512, w_step=256, f0_min=50, f0_max=450, harmo_thresh=0.15, audioDir="./", dataFileName=None, verbose=4):
    """
    ...
    sr, sig = audio_processing.audio_read(audioFilePath, formatsox=True)
...
#    print ("Yin computed in: ", end - start)
    ...
    return pitches, times, harmonic_rates
